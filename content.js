// ============================================================================
// TIB Systems — SAP Testing Execution Guide | Course 1 (Thornfield Industrial Group)
// Supabase-backed content, auth, and progress module
// Project: blfgwysgekfqhcafofhe  (tables: sapexec_books, sapexec_enrollments, sapexec_progress, sapexec_resource_library)
// ============================================================================
...
/** The course-wide Evidence & Templates Library (Policy, Matrix, Tracker, BP-EL Register,
    and one external generic template) — separate from any one Book's sample_exhibit. */
export async function fetchResourceLibrary() {
  const { data, error } = await supabase
    .from('sapexec_resource_library')
    .select('slug, title, subtitle, doc_type, is_external, sort_order, content')
    .order('sort_order', { ascending: true });
  if (error) throw error;
  return data || [];
}
